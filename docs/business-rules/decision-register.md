...
| D31 | Accepted | The timeout demonstration represents provider acceptance followed by a lost response. The external identity and future callback already exist while the local submission outcome is unknown and inventory remains packed. |
| D32 | Accepted | Provider submissions with an unknown outcome support provider status lookup by the stable request key as well as idempotent resubmission. Reconciliation never bypasses the confirmation webhook. |
| D33 | Accepted | An exact duplicate callback reuses the same external event ID and raw body. Repeated HTTP attempts and application processing must remain harmless. Reusing a provider/external-event identity with different raw body bytes is a conflict: reject it without changing the original receipt or applying a shipment effect. |
...
| D37 | Accepted | Each `ShipmentItem` references its source `Reservation`, not a duplicate `order_item_id`. The reservation provides the order item, product, and warehouse relationship; shipment creation verifies that every reservation belongs to the shipment's order and warehouse. |
| D38 | Accepted | A shipment is confirmed as one complete composed unit. `Shipment` persists only `pending_handoff` and `shipped` business states. `ShipmentItem` stores its assigned `quantity` and `delivered_quantity`; it does not duplicate shipped quantity. Partial shipment means composing a shipment for less than the packed quantity, while partial delivery advances `delivered_quantity`. |
| D39 | Accepted | `ProviderSubmission` persists `pending`, `accepted`, `unknown`, and `permanently_failed`. `pending` means the stable request identity exists without a provider result; `accepted` records provider acceptance without marking the shipment shipped; `unknown` means a timeout left the outcome unknown and the same key must be reconciled or replayed; `permanently_failed` permits a new submission. The provider boundary uses the matching accepted, unknown, and permanently-failed outcomes. |
| D40 | Accepted | `MockProviderShipment` persists `accepted`, `permanently_rejected`, `handoff_confirmed`, and `delivered`. `MockProviderWebhook` persists `pending`, `delivering`, `retry_scheduled`, `acknowledged`, and `permanently_failed`; due work is derived from status plus `next_delivery_at`, while a `delivering` row whose `last_attempted_at` is older than the configured delivery-lease cutoff is recoverable. `ProviderWebhookReceipt` persists `pending`, `processed`, `ignored_as_stale`, `retryable_failure`, and `permanently_failed`. Exact replay reuses the same mock-provider webhook row, external event ID, and immutable raw body while increasing its delivery-attempt count; it does not create replay-linkage records. |
