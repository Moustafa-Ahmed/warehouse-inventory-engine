...
It has its own persisted shipment state, its own mock-provider webhook history, and a configured HTTP callback URL. The warehouse application communicates with it only through the `ShippingProvider` contract and the signed webhook endpoint.

The mock provider must not:

- Change warehouse balances directly.
- Mark the warehouse application's `Shipment` as shipped or delivered directly.
...
There is no ordinary administrator action that directly marks a shipment shipped.
...
- A unique mock-provider shipment ID.
- The warehouse application shipment reference supplied in the request.
...
Each mock-provider webhook stores:

- A unique external event ID.
- The mock-provider shipment ID and provider request key.
...
1. The warehouse application durably creates or reuses a `ProviderSubmission` and stable request key.
...
- A successful `2xx` response acknowledges the mock-provider webhook.
- Network errors, timeouts, `429`, and retryable server errors retain the mock-provider webhook for bounded retry.
...
- Every mock-provider webhook has a persisted `next_delivery_at`.
...
7. The later signed callback remains the only webhook allowed to mark the shipment shipped.

Provider status lookup may resolve whether a submission was accepted or rejected, but it does not bypass the webhook-driven shipment transition. If the provider reports confirmed handoff and the callback is missing, the mock provider makes the existing confirmation webhook deliverable again.
...
- “Replay last webhook.”
- “Send out-of-order delivery webhook.”
...
- “Send shipment confirmation now” creates or releases a mock-provider webhook; it never calls the shipment-confirmation service directly.
- Ordinary delivery confirmation requires provider handoff confirmation.
- The explicit out-of-order control is allowed to violate that order so pending-webhook-receipt recovery can be demonstrated.
- Replay keeps the original external event ID and raw body, proving provider-webhook-receipt and domain idempotency.
...
- Provider submission and shipment status.
- Stable provider request key in a safe display form.
- Mock-provider shipment status.
- Outbound callback delivery state and attempt count.
- Provider webhook receipt processing state.
